...
import { useDeliveryQuery } from "../store/apiSlice/apiSlice";
    ...
    render: (row) => (
      <div
        className={`flex items-center justify-center py-2 px-3 rounded-md ${
          row.availability === 1 || row.availability === "available"
...
const Delivery = () => {
  ...
  const { data: deliveryResponse, isFetching } = useDeliveryQuery({
    search: searchInput || undefined,
    date: date ? format(date, "yyyy-MM-dd") : undefined,
    status: selectValue || undefined,
  });
  const location = useLocation();
  const condition = location.pathname.endsWith("add-delivery-employee");

  const hasFilters =
    searchInput || date || (selectValue && selectValue !== "all");
  const deliveryData = hasFilters
    ? deliveryResponse
    : deliveryResponse?.delivery_people;

  const transformedTableBody = Array.isArray(deliveryData)
    ? deliveryData.map((delivery) => {
        return {
          id: delivery?.id || delivery.id,
          name: delivery?.user?.name || delivery.name,
          phone: delivery?.user?.phone || delivery.phone,
          email: delivery?.user?.email || delivery.email,
          orders: delivery?.user?.name || delivery.name,
          vehicleType: delivery?.vehicle_type || delivery.vehicle_type,
          availability: delivery?.is_available || "",
          status: delivery?.user?.status || delivery.status,
          gender: delivery?.user?.gender || delivery.gender,
          photo: delivery?.user?.photo || "",
          created_at: delivery?.user?.created_at
            ? format(delivery.user.created_at, "yyyy-MM-dd")
            : format(delivery.created_at, "yyyy-MM-dd"),
        };
      })
    : [];
  return (
      ...
      <main className=" text-(--primaryFont) p-10 ">
        <header className="flex items-center justify-between   font-bold mb-5">
          <span className="text-sm text-center sm:text-2xl ">
            {condition ? " Add Delivery Employees" : "Delivery Employees List"}
          </span>
          ...
          overflow-hidden max-h-[400px]  hover:overflow-y-scroll custom-scrollbar    transition-all"
                >
                  <TableComponent
                    ...
                    tableBody={transformedTableBody}
