...
// Default values for a new branch form
const defaultValues = {
  description: "",
  location_note: "",
  phone: "",
...
const AddBranch = () => {
  ...
  // Dynamic field array hooks from React Hook Form
  const {
  ...
  const onSubmit = (data) => {
    ...
    alert(
      "New branch created successfully! Check the console for the data object."
    );
    ...
    <main className="p-4 sm:p-6 md:p-10 bg-gray-50 min-h-screen">
      <header className="flex items-center justify-between font-bold mb-8">
        ...
        <div className="flex items-center text-sm sm:text-base gap-1 sm:gap-2 font-medium text-(--secondaryFont)">
          <span
            onClick={() => navigate(-1)}
            className="hover:text-(--primary) text-(--primaryFont) transition-colors cursor-pointer"
          >
            Branches
          </span>
                    ...
                    <FormItem>
                      <div className="grid grid-cols-3 gap-4 items-center">
                        <FormLabel className="text-(--secondaryFont) col-span-1">
                          {defaultValues.working_hours[index].day}
                        </FormLabel>
                        <FormControl>
                          <Input
                            {...inputField}
